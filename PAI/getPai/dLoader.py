...
class networkRoadDataset():
    ...
    def getNeighborBatch(self, roads, adj, level=1):
        self.__fetchData__()
        road_len = len(roads)
        _max = []
        _mean = []
        ...
        for k in range(road_len):
            ...
            selfInfo = self.roadDataSet[roads[k]]
            for j in range(level):
                ...
                for idx in index:
                    if np.sum(new_adj[idx]) == 0:
                        ...
                        index = np.nonzero(new_adj[idx])
                        neighbors = neighbors | set(index[0])
                    else:
                        break
                index = neighbors - last_index
                if not index:
                    break
                for idx in index:
                    neighbour = self.roadDataSet[roads[idx]]
                    neighbour_renorm = (neighbour['x'] + neighbour['y_mean']) * neighbour['y_max']
                    neighbour_renorm = neighbour_renorm / entity['y_max'] - entity['y_mean']
                    x_tp.append(neighbour_renorm)
        ...
        for j in range(self.max_row):
            ...
            for k in range(road_len):
                x_data = roadDataSet[roads[k]]
                x_image = torch.from_numpy(x_data[:, _x_start:_x_end]).contiguous().view(-1, self.in_window)
                _x.append(x_image.numpy())
                #_x.append(x_data[:, _x_start:_x_end]) #[roads, h, w]
            #yield (_x, torch.from_numpy(np.stack(_y, 0)), _max, _mean)
            all_data.append(_x)
        return all_data
